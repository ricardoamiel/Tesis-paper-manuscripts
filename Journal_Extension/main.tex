...
\begin{abstract}
\journal{%
Transformer-based architectures dominate bi-temporal building change detection
(BCD) from very high-resolution (VHR) satellite imagery, yet their multi-head
self-attention (MHSA) modules impose quadratic computational cost that constrains
scalability. This work introduces \textbf{RALA-ChangeFormer}, an efficient variant
of ChangeFormer that replaces every encoder MHSA block with rank-augmented linear
attention (RALA), reducing complexity from $\mathcal{O}(N^2)$ to
$\mathcal{O}(N)$ without altering any other architectural component. On the
original LEVIR-CD and DSIFN-CD benchmarks, RALA-ChangeFormer achieves
$F_1{=}89.87\%$ and $84.07\%$—within training variance of the ChangeFormer
baseline ($90.40\%$ and $86.67\%$)—while reducing attention-related GFLOPs by
$8\%$, peak GPU memory by $22\%$, and increasing the maximum feasible batch size
by $50\%$ at $256{\times}256$ resolution. This journal extension evaluates the
model across four additional benchmarks spanning diverse acquisition conditions:
WHU-CD ($F_1{=}87.85\%$), LEVIR-CD+ ($F_1{=}80.24\%$), SYSU-CD
($F_1{=}78.02\%$), and S2Looking ($F_1{=}60.39\%$). Results reveal a
performance gradient correlated with acquisition geometry and scene
heterogeneity: the model generalises reliably to nadir-view datasets with
homogeneous annotations but degrades markedly under oblique side-looking
geometry (S2Looking) and multi-temporal illumination variation (SYSU-CD), with
S2Looking showing a $-27.5\,\mathrm{pp}$ gap relative to WHU-CD. These findings
position RALA-ChangeFormer as a practical efficiency contribution complementary
to accuracy-maximisation strategies, and motivate future work on geometric
alignment, dynamic rank adaptation, and dataset creation for heterogeneous urban
...
across the four extended benchmarks.
...
& \textbf{Prec} & \textbf{Rec} & \textbf{F1} & \textbf{IoU} & \textbf{OA} \\
\hline
LEVIR-CD+ & 79.97 & 81.34 & 80.65 & 67.58 & 98.44 &
\textbf{80.20} & 80.29 & 80.24 & 67.01 & 98.39 \\

SYSU-CD & 77.90 & 79.74 & 78.81 & 65.03 & 90.12 &
\textbf{82.10} & 74.34 & 78.02 & 63.97 & 90.12 \\

WHU-CD & 84.87 & 90.20 & 87.45 & 77.70 & 99.11 &
\textbf{90.14} & 85.67 & \textbf{87.85} & \textbf{78.33} & 99.06 \\

S2Looking & 72.14 & 54.19 & 61.89 & 45.77 & 84.55 &
...
\noindent{\textbf{LEVIR-CD+ (moderate, in-distribution).}}
...
\noindent{\textbf{SYSU-CD (moderate difficulty, illumination and multi-class complexity).}}
...
\noindent{\textbf{S2Looking (hardest, geometric out-of-distribution).}}
